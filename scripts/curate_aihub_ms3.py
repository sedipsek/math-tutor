#!/usr/bin/env python3
"""Curate known AI Hub MS3 annotation defects without inventing content.

Recoverable transcription/label defects are corrected. Problems missing source
content (choices or explanations) are retained for provenance but quarantined
from ingestion via the `quality` field.
"""
...
ROOT = Path(__file__).resolve().parents[1] / "datasets" / "aihub-ms3"

# Each replacement was reviewed against the complete crop text. These are not
# broad OCR substitutions: the key identifies one exact problem/crop.
...
# AI Hub OCR occasionally emits standalone Hangul choseong (U+1100 block).
# Those code points are composition components, not printable list labels.
...
def main() -> None:
    ...
    for file in files:
        ...
        # A wrong-answer annotation is decisive evidence of multiple choice.
        ...
        # Remove only demonstrably blank bitmap annotations.
        kept = []
        for crop in problem["crops"]:
            if crop.get("path"):
        ...
        has_wrong = any(c["slug"].startswith("wrong") and (c.get("text") or "").strip() for c in kept)
        has_explanation = any(c["slug"].startswith("explanation") and (c.get("text") or "").strip() for c in kept)
        issues = []
        ...
        file.write_text(json.dumps(problem, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        stem_images = [c for c in kept if c["slug"] == "stem_image" and c.get("path")]
        manifest.append({
            "id": pid, "question_type": problem["question_type"],
            "difficulty": problem["difficulty"], "semester": problem["semester"],
            "publisher": problem["publisher"], "unit_codes": problem["unit_codes"],
            "topic_codes": problem["topic_codes"], "n_stem_images": len(stem_images),
            "has_stem_image": bool(stem_images), "quality": problem["quality"],
            "path": f"problems/{pid}/problem.json",
        })

    # Hard validation: a curated ready item must render and fulfill the MVP contract.
    failures = []
    for row, file in zip(manifest, files, strict=True):
        problem = json.loads(file.read_text(encoding="utf-8"))
        if row["quality"]["status"] == "ready":
            for crop in problem["crops"]:
                if odd_dollars(crop.get("text") or ""):
                    failures.append(f"{problem['id']}: odd $ in {crop['slug']}:{crop['index']}")
    if failures:
        raise SystemExit("\n".join(failures))

    (ROOT / "manifest.jsonl").write_text("\n".join(json.dumps(r, ensure_ascii=False) for r in manifest) + "\n", encoding="utf-8")
    (ROOT / "quarantine.json").write_text(json.dumps(quarantined, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ready = [r for r in manifest if r["quality"]["status"] == "ready"]
    stats = {
        "n_problems": len(manifest), "ready": len(ready), "quarantined": len(quarantined),
